...
class CompleteReport(SimpleReport):
    @classmethod
    def returned_companies(cls, product_list):
        final_str = "Produtos estocados por empresa:\n"
        second_data = super().products_per_company(product_list).most_common()
        for company, quantity in second_data:
            final_str += f"- {company}: {quantity}\n"
        return final_str

    @classmethod
    def generate(cls, product_list):
        super().generate(product_list)
        return cls.returned_companies(product_list)


my_inventory = CompleteReport()
print(my_inventory.generate(data))
