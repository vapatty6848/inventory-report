...
class CompleteReport(SimpleReport):
    ...
    def returned_companies(cls, product_list):
        ...
        companies_and_products = super().products_per_company(product_list)
        companies = []
        ...
        for company in companies:
            str_to_be_returned += (
                f"- {company}: {companies_and_products[company]}\n"
            )
        return str_to_be_returned

    @classmethod
    def generate(cls, product_list):

        return super().generate(product_list) + cls.returned_companies(
            product_list
        )


my_inventory = CompleteReport()
print(my_inventory.returned_companies(data))
