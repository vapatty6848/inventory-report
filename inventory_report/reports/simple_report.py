...
most_products_text = "Empresa com maior quantidade de produtos estocados:"
...
class SimpleReport:
    ...
    def generate(products):
        ...
        for product in products:
            ...
            company, production_date, due_date = itemgetter(*props_to_get)(
                product
            )
            ...
            earliest_prd_date = (
                min([earliest_prd_date, production_date_datetime])
                if earliest_prd_date
                else production_date_datetime
            )
            ...
            if due_date_is_past_today:
                max_due_date = (
                    min([due_date_datetime, max_due_date])
                    if max_due_date
                    else due_date_datetime
                )

        answer_pieces = [
            f"Data de fabricação mais antiga: {earliest_prd_date.isoformat()}",
            f"Data de validade mais próxima: {max_due_date.isoformat()}",
            f"{most_products_text} {max(products_count)}\n",
        ]
